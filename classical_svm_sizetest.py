...
csv_file_path = './results/'+dim+'/classical_svm_sizetest.csv'
...
d={'size': size, 'PPT_ratio': ppt_ratio, 'acc_train': np.mean(tr_accs), 'accuracy': np.mean(acc_scores), 'f1': np.mean(f1_scores),
'precision': np.mean(prec_scores), 'recall': np.mean(rec_scores), 'balanced_accuracy': np.mean(acc_scores),
'SEP_accuracy': np.mean(sep_scores), 'PPT_accuracy': np.mean(ppt_scores), 'NPPT_accuracy': np.mean(nppt_scores),
'time': np.mean(times)}

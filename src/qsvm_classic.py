...
folder = 'sizetest'
cat_size = 200
csv_file_path='./results/'+dim+'/qsvm_jit_512pca.csv'
...
#jitted_kernel = qjit(kernel_circ)
#end_jit=time.time()
#print('Jit time: ', end_jit-start_jit)
...
# def qkernel(A, B):
#   return jnp.array([[kernel_circ(a,b)[0] for b in B] for a in A])
...
ppt_scores=[]
nppt_scores=[]
sizes=np.arange(600,1001,100)

for size in range(600,1001,100):
   start=time.time()
   ...
   svm = SVC(kernel = 'poly', degree=2).fit(xs_train, y_train)
...
d={'size': sizes, 'PPT_ratio': ppt_ratio, 'acc_train': tr_acc, 'accuracy': acc_scores, 'f1': f1_scores,

...
size=args.size
csv_file_path="../results/"+dim+"/pca_classical_svm_10mod_size"+size+".csv"
...
model_type = SVC()
model_params = [
    {'kernel': ['poly'], 'degree': np.arange(2, 7), 'gamma': np.logspace(-5, 5, num=10), 'C': np.logspace(-5, 5, num=10)},
    {'kernel': ['rbf'], 'gamma': np.logspace(-5, 5, num=10), 'C': np.logspace(-5, 5, num=10)},
    {'kernel': ['sigmoid'], 'gamma': np.logspace(-5, 5, num=10), 'C': np.logspace(-5, 5, num=10)}]
...
#print('Requesting filelock...')
#with lock:
#print('Got filelock')
for i in range(len(models)) :
    print('---- Test Model ', i)
    y_train_pred=models[i].predict(xs_train)
    start=time.time()
    y_test_pred=models[i].predict(xs_test)
    end=time.time()
    print('Prediction time: ', end-start)
    prediction_times=end-start

    tr_accs=accuracy_score(y_train_pred, y_train)
    acc_scores=accuracy_score(y_test_pred, y_test)
    f1=f1_score(y_test, y_test_pred)
    prec_scores=precision_score(y_test, y_test_pred)
    rec_scores=recall_score(y_test, y_test_pred)
    bacc_scores=balanced_accuracy_score(y_test, y_test_pred)
    acc_per_type=detailed_accuracy(y_test_pred, cat_size)
    sep_scores=acc_per_type['SEP_acc']
    ppt_scores=acc_per_type['PPT_acc']
    nppt_scores=acc_per_type['NPPT_acc']

    #performance(y_train_pred, y_train, y_test_pred, y_test,cat_size)

    times=training_times[i]+prediction_times

    d={'size': size, 'PPT_ratio': ppt_ratio, 'acc_train': tr_accs, 'accuracy': acc_scores, 'f1': f1,
